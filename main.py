...
def TF (dodo):
    ...
    dicto={}
    tableau=[]
    #for mots in texte compter le nombre d'occurence de chaque mots
    ...
    for fichier in fichiers:
        ...
        if os.path.isfile(chemin_fichier_entree):
            with open(chemin_fichier_entree, 'r',encoding="utf-8") as files:
                for mots in files:

                    tab=mots.split(" ")
                    for i in range (len(tab)):

                        if tab[i] not in dicto:
                            dicto[tab[i]]=1
                        else:
                            dicto[tab[i]] += 1

        tableau.append(dicto)
